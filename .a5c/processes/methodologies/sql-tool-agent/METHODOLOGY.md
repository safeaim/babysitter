...
# SQL-Tool-over-MCP for Curated Domain Agents

## Attribution

Source: LinkedIn post by Michael Lugassy (mluggy), 2026.
...
## Generalized 5-step methodology

1. **Ingest.** Identify authoritative data sources, gather them offline, normalize. Do not rely on runtime web-fetch.
2. **Schema.** Design a small, query-friendly relational schema with explicit join keys. Prefer denormalization where it collapses common query shapes.
...
4. **Tool registration.** Register a SQL-execution tool with the LLM harness. Include schema description in the system prompt. Do not wrap in MCP unless multi-client access is required.
5. **Acceptance.** A fixed set of 5–10 natural-language queries the agent must handle correctly with stable output shape. Binary pass/fail gate.
...
- Structured data domains with clear entities and join keys.
- Read-heavy workloads (query, compare, recommend).
- Single-user or small-team usage where MCP multi-client dispatch is unnecessary.
- Stable schema that does not change per query.

## When NOT to use

- Workloads requiring writes or transactional side effects.
- Agents that must compose multiple heterogeneous tools per step.
- Domains with unstable or rapidly evolving schema.
- Multi-client scenarios where MCP's protocol boundary is actually load-bearing.
