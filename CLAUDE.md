...
6. Separately:
   ...
   - **agent-mux-tui** with babysitter TUI plugins provides live observability (status, cost, governance).
...
- Harness CLI entry: `packages/babysitter-agent/src/cli/main.ts`
...
### Hooks Proxy (`packages/hooks-mux/`)

```bash
# Build all hooks-mux packages
npm run build:hooks-mux

# Test all hooks-mux packages
npm run test:hooks-mux

# Lint hooks-mux (core + cli)
npm run lint:hooks-mux

# Individual package commands
cd packages/hooks-mux/core && npx tsc --noEmit    # Type check
cd packages/hooks-mux/core && npx vitest run       # Tests
```

### Babysitter Harness (`packages/babysitter-agent` / `@a5c-ai/babysitter-agent`)

```bash
npm run build --workspace=@a5c-ai/babysitter-agent    # tsc → dist/
npm run clean --workspace=@a5c-ai/babysitter-agent    # rimraf dist
npm run test --workspace=@a5c-ai/babysitter-agent     # vitest run (all tests)
```

### Babysitter TUI Plugins (`packages/babysitter-tui-plugins` / `@a5c-ai/babysitter-tui-plugins`)

```bash
npm run build --workspace=@a5c-ai/babysitter-tui-plugins    # tsc --build
npm run clean --workspace=@a5c-ai/babysitter-tui-plugins    # rimraf dist
npm run test --workspace=@a5c-ai/babysitter-tui-plugins     # vitest run
```
...
| `packages/babysitter-agent` | `@a5c-ai/babysitter-agent` | Optional harness runtime CLI: Pi + agent-mux integration, session/cost/governance, daemon, observability. |
| `packages/babysitter-tui-plugins` | `@a5c-ai/babysitter-tui-plugins` | Status, cost, governance TUI plugins for agent-mux-tui. |
| `packages/hooks-mux/core` | `@a5c-ai/hooks-mux-core` | Hook normalization, session store, merge engine, propagation, SDK interface. |
| `packages/hooks-mux/cli` | `@a5c-ai/hooks-mux-cli` | CLI entrypoint (`a5c-hooks-mux`): invoke, exec, bootstrap, doctor. |
| `packages/hooks-mux/adapter-*` | `@a5c-ai/hooks-mux-adapter-*` | 9 harness adapters (claude, codex, gemini, copilot, cursor, pi, oh-my-pi, opencode, openclaw). |
...
The SDK also provides a unified adapter at `packages/sdk/src/harness/unified/` that delegates to hooks-mux via subprocess, reading `AGENT_CAPABILITIES_JSON` from env for capability derivation. Used as fallback when no harness-specific adapter is detected.
...
## Hooks Proxy Architecture (`packages/hooks-mux/`)
...
- **`cli/`** — CLI entrypoint (`a5c-hooks-mux`): `invoke` (primary hook proxy), `exec` (context-enriched command execution), `bootstrap` (no-op session init), `show-session`, `clear-session`, `doctor`.
...
### Environment Variables (hooks-mux)

| Variable | Description |
|----------|-------------|
| `AGENT_SESSION_ID` | Session ID (set by hooks-mux, read by handlers) |
...
## Babysitter Harness Architecture (`packages/babysitter-agent/`)

Optional harness runtime providing Pi integration, agent-mux bridge, session management, cost tracking, governance, and daemon mode. External harness invocation goes through `@a5c-ai/agent-mux` (no direct subprocess spawning); only Pi and internal harnesses use direct invocation. Supports `--output-format amux-events` for JSONL agent-mux event protocol output.

- **`harness/amux/`** — Agent-mux bridge: `amuxBridge` (delegates external harness invocations to AmuxClient), `amuxClientFactory` (creates AmuxClient instances), `amuxEventEmitter` (streams events to callers), `amuxEventMapper` (maps AmuxAgentEvent to BabysitterEvent), `amuxHarnessMap` (maps babysitter harness names to agent-mux adapter names), `amuxStdinReader` (reads JSONL from stdin for piped interaction), `amuxTypes` (interface-first types matching agent-mux API surface).
- **`harness/invoker.ts`** — Pi/internal direct invocation only; external harnesses route through agent-mux via `amuxBridge`.
- **`harness/piWrapper/`** — Programmatic Pi session management (unchanged from SDK).
- **`session/`** — Session context, persistence, cost tracking, continuity state, memory extraction, history.
- **`cost/`** — Cost collection and parsing (Claude Code parser, generic types).
- **`governance/`** — Policy engine, mandate enforcement, authority, categories, sandbox policy/bridge, permission events/propagation, decision trail, built-in policies.
- **`daemon/`** — Background daemon: loop, file watcher, timer scheduler, webhook listener, structured logging.
- **`observability/`** — Timeline, health checks, webhooks.
- **`interaction/`** — Terminal-based user question prompts.
- **`api/`** — Effects, breakpoints, event stream APIs.
- **`cli/`** — CLI entry (`babysitter-harness`): harness commands (createRun, resumeRun), session history, daemon, TUI, MCP serve, JSONL interactive mode, `amuxEventsFormatter` (formats results as amux-events JSONL).
- **`anycli/`** — Generic CLI adapter types.

Key dependency: `@a5c-ai/agent-mux` (agent multiplexer for external harness invocation).

### Babysitter Harness Commands
...
## Babysitter TUI Plugins (`packages/babysitter-tui-plugins/`)

TUI plugins for agent-mux-tui providing babysitter-specific dashboard panels. ESM module with React/Ink components.

- **`status-plugin.tsx`** — Run status panel for agent-mux TUI dashboard.
- **`cost-plugin.tsx`** — Cost tracking panel.
- **`governance-plugin.tsx`** — Governance/policy panel.
- **`data.ts`** — Data fetching utilities for TUI plugins.
- **`types.ts`** — Shared type definitions.

Peer dependency: `@a5c-ai/agent-mux-tui` (provides the TUI plugin host).

### Babysitter TUI Plugins Commands
