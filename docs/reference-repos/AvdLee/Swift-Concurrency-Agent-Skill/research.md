...
## Plugin Ideas
- **Swift Concurrency Advisor plugin**: Install.md-driven plugin that adds a swift-concurrency skill with reference files for actors, sendable, threading, testing, and linting. Includes project-settings detection hook that auto-reads Package.swift on session start.
- **Language Migration Framework plugin**: Generic plugin template for encoding language-version migration expertise (Swift 6, Python 2->3, Java 8->21) with diagnostic tables and incremental fix workflows.
