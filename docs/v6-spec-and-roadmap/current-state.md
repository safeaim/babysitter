...
V6 does not treat this monolith as ready for broad extraction yet. The first required slice is an internal seam contract inside `@a5c-ai/agent-platform`, not an immediate package split.

That seam contract lives at `packages/agent-platform/src/seams/contract.ts` and assigns each current top-level runtime domain to an owned slice:
...
The contract is only useful if it stays executable. Changes to `packages/agent-platform/src` should therefore keep the seam manifest, package subpath exports, and seam validation tests aligned before any broader extraction is proposed.
