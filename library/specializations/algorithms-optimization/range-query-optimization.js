...
export async function process(inputs, ctx) {
  ...
  let comparison = await ctx.task(rangeQueryComparisonTask, { queryType, implementation, outputDir });
    let lastFeedback = null;
  for (let attempt = 0; attempt < 3; attempt++) {
    if (lastFeedback) {
      comparison = await ctx.task(rangeQueryComparisonTask, { ...{ queryType, implementation, outputDir }, feedback: lastFeedback, attempt: attempt + 1 });
    }
