...
export async function process(inputs, ctx) {
  ...
  let qualityAssessment = await ctx.task(emailAutomationQualityTask, { journeyMapping, nurtureSequences, lifecycleCampaigns, automationRules, personalizationStrategy, outputDir });
  artifacts.push(...qualityAssessment.artifacts);

    let lastFeedback = null;
  for (let attempt = 0; attempt < 3; attempt++) {
    if (lastFeedback) {
      qualityAssessment = await ctx.task(emailAutomationQualityTask, { ...{ journeyMapping, nurtureSequences, lifecycleCampaigns, automationRules, personalizationStrategy, outputDir }, feedback: lastFeedback, attempt: attempt + 1 });
    }
