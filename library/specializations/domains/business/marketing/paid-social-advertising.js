...
export async function process(inputs, ctx) {
  ...
  let qualityAssessment = await ctx.task(paidSocialQualityTask, { platformStrategy, audienceStrategy, campaignStructure, creativeStrategy, biddingOptimization, outputDir });
  artifacts.push(...qualityAssessment.artifacts);

    let lastFeedback = null;
  for (let attempt = 0; attempt < 3; attempt++) {
    if (lastFeedback) {
      qualityAssessment = await ctx.task(paidSocialQualityTask, { ...{ platformStrategy, audienceStrategy, campaignStructure, creativeStrategy, biddingOptimization, outputDir }, feedback: lastFeedback, attempt: attempt + 1 });
    }
