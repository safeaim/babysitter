...
export async function process(inputs, ctx) {
  ...
  let qualityAssessment = await ctx.task(vocProgramQualityTask, { programDesign, feedbackChannels, analysisFramework, actionFramework, outputDir });
  artifacts.push(...qualityAssessment.artifacts);

    let lastFeedback = null;
  for (let attempt = 0; attempt < 3; attempt++) {
    if (lastFeedback) {
      qualityAssessment = await ctx.task(vocProgramQualityTask, { ...{ programDesign, feedbackChannels, analysisFramework, actionFramework, outputDir }, feedback: lastFeedback, attempt: attempt + 1 });
    }
