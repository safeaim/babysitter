...
export async function process(inputs, ctx) {
  ...
  let controlTechAnalysis = await ctx.task(controlTechAnalysisTask, {
    facilityName,
    emissionSources,
    emissionInventory,
    regulatoryApplicability,
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    if (lastFeedback_phase3Review) {
      controlTechAnalysis = await ctx.task(controlTechAnalysisTask, { ...{
    facilityName,
    emissionSources,
    emissionInventory,
    regulatoryApplicability,
  ...
  let permitApplication = await ctx.task(permitApplicationTask, {
    facilityName,
    facilityType,
    permitType,
    emissionSources,
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    if (lastFeedback_finalApproval) {
      permitApplication = await ctx.task(permitApplicationTask, { ...{
    facilityName,
    facilityType,
    permitType,
    emissionSources,
