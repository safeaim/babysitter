 ...
 *   skillAreas: [skill-area:protocol-design, skill-area:socket-programming, skill-area:api-doc-generation]
...
export async function process(inputs, ctx) {
  ...
  let validation = await ctx.task(validationTask, { projectName, features, results, outputDir });
    let lastFeedback = null;
  for (let attempt = 0; attempt < 3; attempt++) {
    if (lastFeedback) {
      validation = await ctx.task(validationTask, { ...{ projectName, features, results, outputDir }, feedback: lastFeedback, attempt: attempt + 1 });
    }
