...
export async function process(inputs, ctx) {
  ...
  let errorHandlingTests = await ctx.task(errorHandlingTestsTask, { projectName, outputDir });
    let lastFeedback = null;
  for (let attempt = 0; attempt < 3; attempt++) {
    if (lastFeedback) {
      errorHandlingTests = await ctx.task(errorHandlingTestsTask, { ...{ projectName, outputDir }, feedback: lastFeedback, attempt: attempt + 1 });
    }
  const finalApproval = await ctx.breakpoint({ question: `API integration testing setup complete for ${projectName}. Approve?`, title: 'API Testing Review', context: { runId: ctx.runId, suites: integrationTests.suites }, expert: 'owner', tags: ['approval-gate'], previousFeedback: lastFeedback || undefined, attempt: attempt > 0 ? attempt + 1 : undefined });
