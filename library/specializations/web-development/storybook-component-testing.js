...
export async function process(inputs, ctx) {
  ...
  let interactionTests = await ctx.task(interactionTestsTask, { projectName, outputDir });
    let lastFeedback = null;
  for (let attempt = 0; attempt < 3; attempt++) {
    if (lastFeedback) {
      interactionTests = await ctx.task(interactionTestsTask, { ...{ projectName, outputDir }, feedback: lastFeedback, attempt: attempt + 1 });
    }
  const finalApproval = await ctx.breakpoint({ question: `Storybook setup complete for ${projectName}. Approve?`, title: 'Storybook Review', context: { runId: ctx.runId, stories: storyPatterns.stories }, expert: 'owner', tags: ['approval-gate'], previousFeedback: lastFeedback || undefined, attempt: attempt > 0 ? attempt + 1 : undefined });
