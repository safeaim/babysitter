...
export async function process(inputs, ctx) {
  ...
  const finalApproval = await ctx.breakpoint({ question: `WebSocket implementation complete for ${projectName}. Approve?`, title: 'WebSocket Review', context: { runId: ctx.runId, channels: channelsSetup.channels }, expert: 'owner', tags: ['approval-gate'], previousFeedback: lastFeedback || undefined, attempt: attempt > 0 ? attempt + 1 : undefined });
