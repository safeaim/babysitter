...
export class ClaudeAdapter extends BaseAgentAdapter {
  ...
  get capabilities(): AgentCapabilities {
    ...
    return {
      ...
      runtimeHooks: {
        ...
        postToolUseFailure: ((hooks as any).postToolUseFailure ?? 'nonblocking') as any,
        postToolBatch: ((hooks as any).postToolBatch ?? 'blocking') as any,
        sessionStart: (hooks.sessionStart ?? 'nonblocking') as any,
        sessionEnd: (hooks.sessionEnd ?? 'nonblocking') as any,
        setup: ((hooks as any).setup ?? 'nonblocking') as any,
        instructionsLoaded: ((hooks as any).instructionsLoaded ?? 'nonblocking') as any,
        configChange: ((hooks as any).configChange ?? 'blocking') as any,
        stop: (hooks.stop ?? 'nonblocking') as any,
        stopFailure: ((hooks as any).stopFailure ?? 'nonblocking') as any,
        userPromptSubmit: (hooks.userPromptSubmit ?? 'nonblocking') as any,
        userPromptExpansion: ((hooks as any).userPromptExpansion ?? 'blocking') as any,
        taskCreated: ((hooks as any).taskCreated ?? 'blocking') as any,
        taskCompleted: ((hooks as any).taskCompleted ?? 'blocking') as any,
        messageDisplay: ((hooks as any).messageDisplay ?? 'nonblocking') as any,
      },
  ...
  async readConfig(_cwd?: string): Promise<import('@a5c-ai/agent-comm-mux').AgentConfig> {
  ...
  async writeConfig(config: Partial<import('@a5c-ai/agent-comm-mux').AgentConfig>, _cwd?: string): Promise<void> {
    const filePath = this.configSchema.configFilePaths?.[0];
    if (!filePath) return;
