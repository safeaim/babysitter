...
@pytest.mark.asyncio
async def test_non_streaming_response_structure(client: AsyncClient, config: ProxyConfig) -> None:
...
@pytest.mark.asyncio
async def test_auth_bearer_token(client: AsyncClient, config: ProxyConfig) -> None:
    with patch("litellm.acompletion", new_callable=AsyncMock, return_value=_mock_response()):
        resp = await client.post(
            "/v1/messages",
            json={
