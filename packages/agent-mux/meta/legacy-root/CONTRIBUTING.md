...
- `packages/core` — `@a5c-ai/agent-comm-mux`
...
2. **Bump every package.**
   ...
   - Update every workspace cross-dependency pin (`@a5c-ai/agent-comm-mux`, `-adapters`, `-cli`) to the new version in the packages that depend on them.
