...
export function projectWearState(state: GatewayStoreState, previous?: WearStateProjection): WearStateEnvelope {
  const full: WearStateProjection = {
  ...
  const diff: Partial<WearStateProjection> = {};
  ...
  return {
    full,
    diff,
    byteLength: new TextEncoder().encode(encoded).length,
  };
}
