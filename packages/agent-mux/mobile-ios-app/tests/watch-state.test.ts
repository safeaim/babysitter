...
import { projectWatchState } from '../src/projection/watchState.js';

describe('watch state projection', () => {
  it('keeps diffs bounded for a compact watch payload', () => {
    const state = {
      runs: {
        byId: {
          'run-1': {
    ...
    const envelope = projectWatchState(state);
