...
public struct SessionSubscribeFrame: Codable {
  public let type: String
  public let sessionId: String
}

public struct SessionUnsubscribeFrame: Codable {
  public let type: String
  public let sessionId: String
}

public struct SessionStartFrame: Codable {
  public let type: String
...
public enum ClientFrame: Codable {
  ...
  case sessionSubscribe(SessionSubscribeFrame)
  case sessionUnsubscribe(SessionUnsubscribeFrame)
  case sessionStart(SessionStartFrame)
  case sessionMessage(SessionMessageFrame)
  case ping(PingFrame)
  ...
  public init(from decoder: Decoder) throws {
    ...
    case "session.subscribe":
      self = .sessionSubscribe(try SessionSubscribeFrame(from: decoder))
    case "session.unsubscribe":
      self = .sessionUnsubscribe(try SessionUnsubscribeFrame(from: decoder))
    case "session.start":
      self = .sessionStart(try SessionStartFrame(from: decoder))
    case "session.message":
      self = .sessionMessage(try SessionMessageFrame(from: decoder))
    case "ping":
  ...
  public func encode(to encoder: Encoder) throws {
    ...
    case .unsubscribe(let value):
      try value.encode(to: encoder)
    case .sessionSubscribe(let value):
      try value.encode(to: encoder)
    case .sessionUnsubscribe(let value):
      try value.encode(to: encoder)
    case .sessionStart(let value):
      try value.encode(to: encoder)
    case .sessionMessage(let value):
      try value.encode(to: encoder)
    case .ping(let value):
      try value.encode(to: encoder)
