...
export function verifyAgentMuxWebuiRelease({ packageRoot, manifest, packEntries }) {
  ...
  expect(
    scripts['build:realtime'] === 'npm run build --workspace=@a5c-ai/agent-catalog && npm run build --workspace=@a5c-ai/agent-mux-core && npm run build --workspace=@a5c-ai/agent-mux-ui && npm run build',
