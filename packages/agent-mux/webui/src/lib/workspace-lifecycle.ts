...
type ManagedWorkspaceRecord = ManagedWorkspaceSummary | WorkspaceRecord;

function matchesManagedWorkspacePath(
  workspace: ManagedWorkspaceRecord,
  targetPath: string,
  resolveWorkspaceDefaultCwd: (workspace: ManagedWorkspaceRecord) => string,
): boolean {
  const normalized = normalizeWorkspacePath(targetPath);
  if (normalized === workspace.rootPath || normalized === resolveWorkspaceDefaultCwd(workspace)) {
    return true;
  }
  return workspace.repos.some((repo: ManagedWorkspaceRecord["repos"][number]) =>
  ...
  async listWorkspaces(input: {
    ...
    const scopedManagedWorkspaces = focusWorkspacePath
      ? managedWorkspaces.filter((workspace) =>
          matchesManagedWorkspacePath(workspace, focusWorkspacePath, resolveWorkspaceDefaultCwd),
        )
      : managedWorkspaces;
    ...
    for (const session of scopedSessions) {
      ...
      const managedWorkspace = managedWorkspaces.find((workspace) =>
        matchesManagedWorkspacePath(workspace, session.cwd!, resolveWorkspaceDefaultCwd),
      );
