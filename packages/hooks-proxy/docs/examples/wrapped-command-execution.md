...
exports.handler = async function(event) {
  ...
  return {
    ...
    toolMutation: {
      mode: 'replace',
      value: {
        command: `npx -y @a5c/hooks-proxy exec --session-id "${sessionId}" -- ${originalCommand}`,
      },
    },
  };
};
...
Note: Tool mutation is only supported on adapters with `supportsToolInputMutation: true`. Check the adapter capabilities with `a5c-hooks-proxy doctor`.
