                  ...
                  {items.map((item) => (
                    <div key={item.issueId} className="rounded-2xl border border-current/10 bg-card/75 p-3">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2 text-xs">
                        {item.primaryRunId ? (
                          <Link href={`/runs/${item.primaryRunId}`} className="inline-flex items-center gap-1 text-primary">
                            Run
                            <ArrowUpRight className="h-3 w-3" />
                          </Link>
                        ) : null}
                        {item.primarySessionId ? (
                          <Link href={`/sessions/${item.primarySessionId}`} className="inline-flex items-center gap-1 text-primary">
                            Session
                            <ArrowUpRight className="h-3 w-3" />
                          </Link>
                        ) : null}
        ...
        <div className="space-y-4">
            ...
            <div className="mt-4 space-y-3">
              {health.map((item) => (
                <div key={item.runId} className="rounded-2xl border border-border bg-card p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-xs">
                    <Link href={item.recoveryHref} className="inline-flex items-center gap-1 text-primary">
                      {item.stuck || item.status === "failed" ? <LifeBuoy className="h-3 w-3" /> : <Clock3 className="h-3 w-3" />}
                      Recovery
                    </Link>
                    {item.reviewHref ? (
                      <Link href={item.reviewHref} className="inline-flex items-center gap-1 text-primary">
          ...
          <article className="rounded-2xl border border-border bg-background/80 p-4">
            ...
            <div className="mt-4 grid gap-3 text-sm text-foreground-muted">
              ...
              <Link href="/workspaces" className="rounded-2xl border border-border bg-card px-4 py-3 text-left hover:border-primary/30 hover:text-foreground">
