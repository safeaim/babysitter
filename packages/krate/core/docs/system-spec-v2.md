...
All 83 resource kinds from `RESOURCE_DEFINITIONS` in `packages/krate/core/src/resource-model.js`.
...
Source: `packages/krate/core/src/krate-inference-service-controller.js`

### 9.1 KrateInferenceService
...
| `spec.predictor.model.runtime` | string | No | Name of a KrateServingRuntime to use |
...
Source: `packages/krate/core/src/artifact-registry-controller.js`
...
Source: `packages/krate/core/src/assistant-runtime.js`
...
Removes session from the in-process store (`globalThis.__krateSessions`). Sessions are ephemeral and do not persist across process restarts.
...
|--------|------|------|-------------|
| GET | `/api/orgs/{org}/inference/services` | none | List KrateInferenceServices |
| POST | `/api/orgs/{org}/inference/services` | withAuth | Create KrateInferenceService |
...
| GET | `/api/orgs/{org}/inference/runtimes` | none | List KrateServingRuntimes |
...
| Domain | Kinds Added |
|--------|------------|
| Inference | KrateInferenceService, KrateServingRuntime |
