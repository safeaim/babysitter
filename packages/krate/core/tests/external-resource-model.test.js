...
describe('kind set counts after external kinds added', () => {
  it('CONFIG_KINDS has 65 members (42 previous + 5 agent identity + 8 external config + 1 webhook config + 2 Jitsi config + 3 artifact config + 2 inference + 1 model route + 1 virtual model)', () => {
    assert.equal(CONFIG_KINDS.size, 65);
  });

  it('AGGREGATED_KINDS has 33 members (23 previous + 6 external aggregated + 2 Jitsi aggregated + 2 artifact aggregated)', () => {
    assert.equal(AGGREGATED_KINDS.size, 33);
  });

  it('ALL_KINDS has 98 members (65 previous + 5 agent identity + 14 external + 1 webhook config + 4 Jitsi + 5 artifact + 2 inference + 1 model route + 1 virtual model)', () => {
