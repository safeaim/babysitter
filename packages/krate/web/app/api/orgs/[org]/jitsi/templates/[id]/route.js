...
export const GET = withAuth(async (_request, { params }) => {
  ...
  try {
    return Response.json(await getJitsiResource(org, 'JitsiMeetingTemplate', id), { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
...
export const PATCH = withAuth(async (request, { params }) => {
  const { org, id } = await params;
  try {
    const existingResult = await getJitsiResource(org, 'JitsiMeetingTemplate', id);
...
export const DELETE = withAuth(async (_request, { params }) => {
  const { org, id } = await params;
  try {
    return Response.json(await deleteJitsiResource(org, 'JitsiMeetingTemplate', id), { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
