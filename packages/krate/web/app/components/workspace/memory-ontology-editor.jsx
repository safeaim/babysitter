      ...
      <div className="card">
        ...
        {nodeKinds.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem' }} role="list" aria-label="Node kinds">
            {nodeKinds.map((kind) => (
              <div key={kind.name} role="listitem" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.5rem 0.75rem', background: 'var(--bg-subtle)', borderRadius: '0.375rem', border: '1px solid var(--border)' }}>
                ...
                <button style={btnDanger} onClick={() => removeNodeKind(kind.name)} aria-label={`Remove node kind ${kind.name}`}>Remove</button>
              </div>
            ))}
          </div>
        )}
        {showNodeForm ? (
          <div style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border)', borderRadius: '0.5rem', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button style={btnPrimary} onClick={addNodeKind} disabled={!newNodeName.trim()} aria-label="Add node kind">Add node kind</button>
              <button style={btnSecondary} onClick={() => setShowNodeForm(false)} aria-label="Cancel adding node kind">Cancel</button>
            </div>
          </div>
        ) : (
          <button style={btnSecondary} onClick={() => setShowNodeForm(true)} aria-label="Add new node kind">+ Add node kind</button>
        )}
      </div>
      ...
      <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem' }} role="list" aria-label="Edge kinds">
            {edgeKinds.map((kind) => (
              <div key={kind.name} role="listitem" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.5rem 0.75rem', background: 'var(--bg-subtle)', borderRadius: '0.375rem', border: '1px solid var(--border)' }}>
                ...
                <button style={btnDanger} onClick={() => removeEdgeKind(kind.name)} aria-label={`Remove edge kind ${kind.name}`}>Remove</button>
              </div>
            ))}
          </div>
        )}
        {showEdgeForm ? (
          <div style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border)', borderRadius: '0.5rem', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button style={btnPrimary} onClick={addEdgeKind} disabled={!newEdgeName.trim()} aria-label="Add edge kind">Add edge kind</button>
              <button style={btnSecondary} onClick={() => setShowEdgeForm(false)} aria-label="Cancel adding edge kind">Cancel</button>
            </div>
          </div>
        ) : (
          <button style={btnSecondary} onClick={() => setShowEdgeForm(true)} aria-label="Add new edge kind">+ Add edge kind</button>
        )}
      </div>
