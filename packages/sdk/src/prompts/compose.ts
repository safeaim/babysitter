...
export function composeBabysitSkillPrompt(ctx: PromptContext): string {
  ...
  return joinNonEmpty([
    ...
    parts.renderBranchPolicies(ctx),
    parts.renderPrPolicies(ctx),
    parts.renderDraftPrProhibition(ctx),
    parts.renderIssueLinking(ctx),
    parts.renderLabelTaxonomy(ctx),
    parts.renderSingleChannelRule(ctx),
    parts.renderSourceQuoteCap(ctx),
    parts.renderHandoffConventions(ctx),
    parts.renderIdempotencyAndAbort(ctx),
    parts.renderIssueOnlyNoDirectCommits(ctx),
    parts.renderPrCommentFormat(ctx),
    parts.renderSixDimensionReview(ctx),
    parts.renderScheduledReportFormat(ctx),
    parts.renderLocalDevRelax(ctx),
