...
export function nodeTask<TArgs = unknown, TResult = unknown>(
  ...
  if (!options || !options.entry) throw new Error("nodeTask requires an entry option");
  return defineTask<TArgs, TResult>(id, async (args, ctx) => {
    ...
    return { kind: "node", title, description, labels, io, metadata, node: nodeOptions };
  }, { kind: "node" });
}
...
export function orchestratorTask<TArgs = JsonRecord, TResult = unknown>(
  id: string, options: OrchestratorTaskDefinitionOptions<TArgs> = {}
): DefinedTask<TArgs, TResult> {
  return defineTask<TArgs, TResult>(id, async (args, ctx) => {
    ...
    return { kind: "orchestrator_task", title: resolvedTitle, description, labels, metadata: mergedMetadata, orchestratorTask: orchestrator };
  }, { kind: "orchestrator_task" });
}

export function sleepTask<TArgs extends SleepTaskBuilderArgs = SleepTaskBuilderArgs>(
  id = DEFAULT_SLEEP_TASK_ID, options: SleepTaskDefinitionOptions<TArgs> = {}
): DefinedTask<TArgs, void> {
  return defineTask<TArgs, void>(id, async (args, ctx) => {
    ...
    if (typeof iso !== "string" || !iso.length) throw new Error("sleepTask requires args.iso to be a non-empty ISO string");
    if (typeof targetEpochMs !== "number" || !Number.isFinite(targetEpochMs)) throw new Error("sleepTask requires args.targetEpochMs to be a finite number");
    ...
    return { kind: "sleep", title: resolvedTitle, description, labels, metadata: mergedMetadata, sleep: { iso, targetEpochMs } };
  }, { kind: "sleep" });
}

function buildDefaultNodeIo(ctx: TaskBuildContext): TaskIOHints {
  return { inputJsonPath: ctx.toTaskRelativePath("inputs.json"), outputJsonPath: ctx.toTaskRelativePath("result.json"), stdoutPath: ctx.toTaskRelativePath("stdout.log"), stderrPath: ctx.toTaskRelativePath("stderr.log") };
}
...
function sanitizeEnv(input?: EnvInput): { env?: Record<string, string>; redacted: string[] } {
  ...
  for (const [rawKey, rawValue] of Object.entries(input)) {
    ...
    if (shouldRedactEnvKey(key)) { redacted.push(key); continue; }
    env[key] = value;
  }
  redacted.sort((a, b) => a.localeCompare(b));
  return { env: Object.keys(env).length ? env : undefined, redacted };
}

function shouldRedactEnvKey(key: string): boolean {
  const upper = key.toUpperCase();
  if (upper === "NODE_AUTH_TOKEN") return true;
...
function appendRedactedEnvKeys(metadata: JsonRecord | undefined, keys: string[]): JsonRecord | undefined {
  ...
  const existing: string[] = Array.isArray(result.redactedEnvKeys) ? (result.redactedEnvKeys as unknown[]).filter((v): v is string => typeof v === "string") : [];
...
function buildNodeOptions(entry: string, args: string[] | undefined, env: Record<string, string> | undefined, cwd: string | undefined, timeoutOverride: number | undefined): NodeTaskOptions {
  const nodeOptions: NodeTaskOptions = { entry };
  if (args !== undefined) nodeOptions.args = args;
  if (env) nodeOptions.env = env;
  if (typeof cwd === "string" && cwd.length) nodeOptions.cwd = cwd;
...
function buildOrchestratorOptions(payload?: JsonRecord, resumeCommand?: string): OrchestratorTaskOptions {
  ...
  if (payload) orchestrator.payload = payload;
  if (resumeCommand) orchestrator.resumeCommand = resumeCommand;
  return orchestrator;
}
