...
This guide covers the current packaged surface for `@a5c-ai/tasks-mux`.
...
`packages/tasks-mux/` is a single workspace package. The published tarball contains only:
...
```json
{
  "mcpServers": {
    "tasks-mux": {
      "command": "npx",
      "args": ["-y", "@a5c-ai/tasks-mux", "server", "start"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "tasks-mux": {
      "command": "node",
      "args": ["packages/tasks-mux/dist/cli/index.js", "server", "start"]
    }
  }
}
```
...
The package also exports HTTP MCP helpers from `@a5c-ai/tasks-mux/mcp`, but HTTP transport is a programmatic API surface, not a separate CLI package or command.
