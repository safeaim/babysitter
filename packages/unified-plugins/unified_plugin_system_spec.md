...
```bash
npx @a5c-ai/unified-plugins compile --target claude-code --output dist/claude-code
npx @a5c-ai/unified-plugins compile --target codex --output dist/codex
npx @a5c-ai/unified-plugins compile --target all --output dist/
```
...
5. **hooks-proxy is a dependency, not internalized**: Hook execution routing goes through the hooks-proxy subsystem. The compiler generates the shell/Node.js/PowerShell wrapper scripts that call into hooks-proxy.
...
my-plugin/
  ...
  versions.json            # SDK + hooks-proxy version pins (required)
...
```jsonc
{
  ...
  "hookConfig": {                                  // object, optional.
    "proxyAdapter": true,                          // boolean. Use hooks-proxy for all hooks.
...
interface A5cPluginManifest {
  ...
  // ── Hook Configuration ────────────────────────────────────────
  hookConfig?: {
    proxyAdapter?: boolean;                         // Use hooks-proxy for all hooks
...
Pinned dependency versions for SDK and hooks-proxy. Used by hook scripts to install the correct versions at runtime.
...
The `sdkVersion` field is used for **both** `@a5c-ai/babysitter-sdk` and `@a5c-ai/hooks-proxy-cli` installation. This is the current behavior in the codebase -- both packages share a version.
...
All hook-capable targets use the same pattern: a **wrapper script** that ensures the SDK and hooks-proxy are installed, then delegates to hooks-proxy with the correct adapter name.
...
# ... (logging helper, SDK install, hooks-proxy install) ...

# Delegate to hooks-proxy
RESULT=$($PROXY invoke \
...
|----------|-------------|---------|
| `{{ADAPTER_NAME}}` | hooks-proxy adapter name | `claude`, `codex`, `gemini`, `copilot`, `cursor`, `opencode`, `openclaw` |
...
### 6.2 hooks-proxy Adapter Names

Each target uses a specific adapter name when calling hooks-proxy:
...
These correspond to the hooks-proxy adapter packages:
`packages/hooks-proxy/adapter-claude/`, `packages/hooks-proxy/adapter-codex/`, etc.

### 6.3 AdapterCapabilities Interface

Each hooks-proxy adapter declares its capabilities via the `AdapterCapabilities` interface:
...
The `versions.json` file is emitted for every target. Hook scripts read it at runtime to determine which version of `@a5c-ai/babysitter-sdk` and `@a5c-ai/hooks-proxy-cli` to install.
...
5. All cross-package dependency references (`@a5c-ai/babysitter-sdk`, `@a5c-ai/hooks-proxy-core`)
...
jobs:
  compile:
    ...
    steps:
      ...
      # Compile all targets
      - run: npx @a5c-ai/unified-plugins compile --target all --output dist/ --verify
...
```bash
npx @a5c-ai/unified-plugins compile --target claude-code --output /tmp/test-claude-code
npx @a5c-ai/unified-plugins diff --target claude-code --existing plugins/babysitter/
```
...
```
npx @a5c-ai/unified-plugins compile --target <name|all> --output <dir> [options]
...
npx @a5c-ai/unified-plugins compile --target all --output dist/ --verify

# Dry run to see what would be emitted
npx @a5c-ai/unified-plugins compile --target gemini --output dist/gemini --dry-run --json
```
...
```
npx @a5c-ai/unified-plugins validate [options]
...
```bash
npx @a5c-ai/unified-plugins validate --source plugins/babysitter-unified --strict
```
...
```
npx @a5c-ai/unified-plugins diff --target <name> --existing <dir> [options]
...
```bash
npx @a5c-ai/unified-plugins diff --target codex --existing plugins/babysitter-codex
```
...
```
npx @a5c-ai/unified-plugins init --name <name> [options]
...
```bash
npx @a5c-ai/unified-plugins init --name my-plugin --template full --output plugins/my-plugin
```
...
```
npx @a5c-ai/unified-plugins list-targets [options]
...
## Appendix B: hooks-proxy Adapter Packages

The hooks-proxy subsystem has per-adapter packages that normalize hook I/O across harnesses:

| Adapter Package | Harness | Family |
|----------------|---------|--------|
| `packages/hooks-proxy/adapter-claude/` | Claude Code | shell-hook |
| `packages/hooks-proxy/adapter-codex/` | Codex | shell-hook |
| `packages/hooks-proxy/adapter-cursor/` | Cursor | shell-hook |
| `packages/hooks-proxy/adapter-gemini/` | Gemini CLI | shell-hook |
| `packages/hooks-proxy/adapter-copilot/` | GitHub Copilot | shell-hook |
| `packages/hooks-proxy/adapter-opencode/` | OpenCode | in-process |
| `packages/hooks-proxy/adapter-openclaw/` | OpenClaw | in-process |
| `packages/hooks-proxy/adapter-pi/` | Pi | in-process |
| `packages/hooks-proxy/adapter-oh-my-pi/` | oh-my-pi | in-process |
...
npx @a5c-ai/unified-plugins compile --target all --source test/fixtures/test-plugin --output test/output --verify
```
...
npx @a5c-ai/unified-plugins compile --target all --output test/snapshots --source test/fixtures/test-plugin

# CI check: compile and compare (fails if output differs from snapshot)
npx @a5c-ai/unified-plugins compile --target all --output /tmp/test-output --source test/fixtures/test-plugin
npx @a5c-ai/unified-plugins diff --target all --existing test/snapshots
```
...
npx @a5c-ai/unified-plugins diff --target codex --existing plugins/babysitter-codex
npx @a5c-ai/unified-plugins diff --target pi --existing plugins/babysitter-pi
npx @a5c-ai/unified-plugins diff --target all --existing plugins/
```
...
| `sdkVersion` | `versions.json` | SDK/hooks-proxy version pin (used at runtime) |
...
Hook scripts read `versions.json` at runtime to determine which SDK/hooks-proxy version to install. If a compiled plugin's `versions.json` is manually edited after compilation:
