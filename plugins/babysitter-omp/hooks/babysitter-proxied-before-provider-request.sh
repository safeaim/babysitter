#!/bin/bash
# Pre-LLM-request interception Hook
#
# Env: ADAPTER_NAME (required), PLUGIN_ROOT (optional, auto-detected)

set -euo pipefail

ADAPTER_NAME="${ADAPTER_NAME:-oh-my-pi}"
...
INPUT_FILE=$(mktemp 2>/dev/null || echo "/tmp/hook-before-provider-request-$$.json")
cat > "$INPUT_FILE"

STDERR_LOG="$LOG_DIR/hook-before-provider-request-stderr.log"
RESULT=$($PROXY invoke \
  --adapter "$ADAPTER_NAME" \
  --handler "babysitter hook:run --harness unified --hook-type before-provider-request --json" \
  --json \
