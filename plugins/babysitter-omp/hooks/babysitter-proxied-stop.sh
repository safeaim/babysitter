#!/bin/bash
# Stop Hook — checks for pending effects. No SDK install (session-start handles that).
#
...
INPUT_FILE=$(mktemp 2>/dev/null || echo "/tmp/hook-stop-$$.json")
cat > "$INPUT_FILE"

STDERR_LOG="$LOG_DIR/hook-stop-stderr.log"
RESULT=$($PROXY invoke \
