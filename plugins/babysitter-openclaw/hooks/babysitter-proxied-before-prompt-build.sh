#!/bin/bash
# Pre-prompt-build interception Hook
#
...
INPUT_FILE=$(mktemp 2>/dev/null || echo "/tmp/hook-before-prompt-build-$$.json")
cat > "$INPUT_FILE"

STDERR_LOG="$LOG_DIR/hook-before-prompt-build-stderr.log"
RESULT=$($PROXY invoke \
  --adapter "$ADAPTER_NAME" \
  --handler "babysitter hook:run --harness unified --hook-type before-prompt-build --json" \
  --json \
