...
INPUT_FILE=$(mktemp 2>/dev/null || echo "/tmp/hook-session-idle-$$.json")
cat > "$INPUT_FILE"

STDERR_LOG="$LOG_DIR/hook-session-idle-stderr.log"
RESULT=$($PROXY invoke \
  --adapter "$ADAPTER_NAME" \
  --handler "babysitter hook:run --harness unified --hook-type session-idle --json" \
  --json \
