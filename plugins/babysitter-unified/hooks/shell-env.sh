#!/bin/bash
# Shell environment injection Hook
#
...
INPUT_FILE=$(mktemp 2>/dev/null || echo "/tmp/hook-shell-env-$$.json")
cat > "$INPUT_FILE"

STDERR_LOG="$LOG_DIR/hook-shell-env-stderr.log"
RESULT=$($PROXY invoke \
  --adapter "$ADAPTER_NAME" \
  --handler "babysitter hook:run --harness unified --hook-type shell-env --json" \
  --json \
