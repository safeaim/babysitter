...
INPUT_FILE=$(mktemp 2>/dev/null || echo "/tmp/hook-pre-compact-$$.json")
cat > "$INPUT_FILE"

STDERR_LOG="$LOG_DIR/hook-pre-compact-stderr.log"
RESULT=$($PROXY invoke \
  --adapter "$ADAPTER_NAME" \
  --handler "babysitter hook:run --harness unified --hook-type pre-compact --json" \
  --json \
