...
INPUT_FILE=$(mktemp 2>/dev/null || echo "/tmp/hook-subagent-stop-$$.json")
cat > "$INPUT_FILE"

STDERR_LOG="$LOG_DIR/hook-subagent-stop-stderr.log"
RESULT=$($PROXY invoke \
  --adapter "$ADAPTER_NAME" \
  --handler "babysitter hook:run --harness unified --hook-type subagent-stop --json" \
  --json \
