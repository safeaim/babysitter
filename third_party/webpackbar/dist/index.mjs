...
const colorize = (color) => {
  ...
  return chalk[color] || chalk.reset;
};
...
class FancyReporter {
  ...
  _renderState(state) {
    ...
    if (state.progress >= 0 && state.progress < 100) {
      ...
      line2 = state.request ? " " + chalk.grey(
...
class WebpackBarPlugin extends Webpack.ProgressPlugin {
  constructor(options) {
    ...
    __publicField(this, "webpackbarOptions");
    ...
    const _reporters = [
      ...this.webpackbarOptions.reporters || [],
      this.webpackbarOptions.reporter
    ...
    if (this.webpackbarOptions.profile && !_reporters.some(({ reporter }) => reporter === "profile")) {
  ...
  get state() {
    return globalStates[this.webpackbarOptions.name];
  }
  _ensureState() {
    if (!this.states[this.webpackbarOptions.name]) {
      this.states[this.webpackbarOptions.name] = {
        ...DEFAULT_STATE,
        color: this.webpackbarOptions.color,
        name: startCase(this.webpackbarOptions.name)
      };
    }
  }
  apply(compiler) {
